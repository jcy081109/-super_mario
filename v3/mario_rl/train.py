"""
训练入口：流水线 PPO + 环境随机化训练马里奥。

v3 核心：双缓冲流水线收集+更新，CPU/GPU 最大化利用 + 随机初始位置/随机帧跳过缓解过拟合。
...
用法：
    ...
    python -m mario_rl.train --random-start --max-start-x 2000 --random-frame-skip
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        解析后的参数命名空间。
    """
    parser = argparse.ArgumentParser(description="马里奥 PPO 训练（v3 流水线+域随机化版）")
    ...
    parser.add_argument("--random-start", action="store_true",
                        help="启用随机初始位置（reset 时从关卡任意 x_pos 开始，防止记路线）")
    parser.add_argument("--max-start-x", type=int, default=2000,
                        help="随机初始位置的最大 x_pos（建议关卡长度的 50~70%%）")
    parser.add_argument("--random-frame-skip", action="store_true",
                        help="启用随机帧跳过（每步 3~5 帧随机，防止记精确时间节奏）")
    parser.add_argument("--min-frame-skip", type=int, default=3, help="随机帧跳过最小值")
    parser.add_argument("--max-frame-skip", type=int, default=5, help="随机帧跳过最大值")
...
def main() -> None:
    ...
    if args.random_start:
        print(f"随机初始位置: 启用（max_x={args.max_start_x}，reset 时从任意位置开始）")
    if args.random_frame_skip:
        print(f"随机帧跳过: 启用（每步 {args.min_frame_skip}~{args.max_frame_skip} 帧随机）")
    ...
    envs = make_vec_envs(
        ...
        max_start_x=args.max_start_x,
        random_frame_skip=args.random_frame_skip,
        min_frame_skip=args.min_frame_skip,
    ...
    if args.multi_level:
        ...
        eval_envs = make_vec_envs(
            ...
            time_penalty=args.time_penalty,
            jump_penalty=args.jump_penalty,
            random_start=False,  # 评估用干净环境，从起点开始
            random_frame_skip=False,  # 评估用固定帧跳过
        )
        print(f"评估环境: {len(eval_worlds)}章×1关(stage4) = {len(eval_worlds)}测试关")
    ...
    try:
        while pipeline.total_timesteps < args.total_timesteps:
            ...
            if total_steps - last_eval >= args.eval_freq:
                ...
                print(f"[评估] 第 {total_steps:,} 步，评估 {config.train.eval_episodes} 局"
                      f"（{'测试关stage4' if eval_envs is not None else '训练关'}）...")
